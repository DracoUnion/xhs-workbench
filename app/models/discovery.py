...
class RankingSource(PKMixin, Base):
    """千帆数据中心 8 个榜单入口配置。"""
...
class Account(PKMixin, TimestampMixin, Base):
    ...
    metrics: Mapped["AccountMetric | None"] = relationship(
        back_populates="account", cascade="all, delete-orphan", uselist=False
    )


class AccountMetric(PKMixin, Base):
    """评分分项明细，便于看板追溯与调参验证。"""
...
class ProductImage(PKMixin, Base):
...
class Direction(PKMixin, Base):
    ...
    picked_product_id: Mapped[int | None] = mapped_column(ForeignKey("products.id"))


class DirectionAccount(PKMixin, Base):
